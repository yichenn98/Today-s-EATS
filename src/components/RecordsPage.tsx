...
const RecordsPage: React.FC<RecordsPageProps> = ({ records, selectedDate, setSelectedDate, onEditRecord, onAddAtCategory }) => {
  const dailyRecords = useMemo(() => {
    return records.filter(r => r.date === selectedDate);
  }, [records, selectedDate]);

  const getMealForCategory = (cat: Category) => {
    return dailyRecords.find(r => r.category === cat);
  };
              ...
              {meal ? (
                ...
                <div className="absolute inset-0 w-full h-full">
                  {meal.image ? (
                    /* 背景：照片 */
                    <>
                      <img src={meal.image} alt={meal.mealName} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"></div>
                    </>
                  ) : (
                    /* 背景：全彩色塊 */
                    <div className="absolute inset-0 w-full h-full flex items-center justify-center" style={{ backgroundColor: color }}>
                ...
                <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center p-4">
                  ...
                  <span className="text-[#5D6D7E]/30 text-[10px] font-black tracking-widest uppercase group-hover:text-[#5D6D7E]/50">{cat}</span>
                </div>
              )}
              
              {/* 左上角類別標籤 */}
              <div className={`absolute top-4 left-4 px-2.5 py-1 rounded-xl shadow-sm border flex items-center gap-1.5 z-20 ${meal ? 'bg-white/20 backdrop-blur-md border-white/10' : 'bg-white/90 border-[#E5DCD3]/30'}`}>
